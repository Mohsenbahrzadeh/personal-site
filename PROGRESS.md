...
## نکته مهم امنیتی — appsettings.json (باز، حل‌نشده)
روی دیسک الان `appsettings.json` یک کانکشن‌استرینگ واقعی به یک SQL Server روی
`127.0.0.1:1433` با یوزر `sa` و یک رمز خام دارد (عمداً اینجا رمز را نمی‌نویسم چون
این فایل ممکن است commit شود) — **این را من ننوشتم**،
روی دیسک پیدا شد (احتمالاً خود کاربر برای تست با یک SQL Server واقعی روی پورت 1433
تنظیم کرده). نسخه‌ی commit‌شده در گیت‌هاب همچنان همان LocalDB امن قبلی است، پس این
رمز عمومی نشده. کاربر گفته «فعلاً همینه، بعداً عوضش می‌کنم» — **این یعنی این موضوع باز
است و باید حواس‌مان باشد این فایل با این حالت commit نشود** تا وقتی کاربر یا رمز را عوض
کند یا به `dotnet user-secrets` منتقلش کند. سشن بعدی حتماً این را چک کند.
