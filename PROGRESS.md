...
- [x] انتشار عمومی روی گیت‌هاب: https://github.com/Mohsenbahrzadeh/personal-site
- [x] بازطراحی کامل ظاهر (`site.css`) — گرادیان بنفش‌آبی، درخشش کارت‌ها/دکمه‌ها، بلاب‌های
  پس‌زمینه، تایپوگرافی و فاصله‌گذاری بهتر در همه صفحات + commit و push شد
- [x] ادیتور متن (rich text) برای فیلد Body در `Admin/Edit` — چون کاربر ابزار HTML
  editor جداگانه نداشت. پیاده‌سازی خانگی با `contentEditable` + نوار ابزار
  (`wwwroot/js/rich-editor.js`)، بدون کتابخانه بیرونی (Quill/TinyMCE رد شد چون دانلود
  خودکار کد شخص ثالث برای commit عمومی نیاز به تأیید کاربر داشت و وقت نبود بپرسم).

## نکته مهم امنیتی — تغییرات دستی روی دیسک (باز، حل‌نشده)
دو فایل به‌صورت مستقل از کار من روی دیسک تغییر کرده‌اند (احتمالاً خود کاربر، برای تست):

1. **`appsettings.json`** — کانکشن‌استرینگ به یک SQL Server واقعی روی `127.0.0.1:1433`
   با یوزر `sa` و رمز خام عوض شده (عمداً رمز را اینجا نمی‌نویسم). نسخه commit‌شده در
   گیت‌هاب همچنان LocalDB امن است.
2. **`Data/SeedData.cs`** — رمز fallback ادمین (وقتی `AdminSeed:Password` ست نشده)
   از یک مقدار تصادفی به رشته‌ی ثابت `Admin@123` تغییر کرده — یعنی اگر این نسخه
   commit شود، دقیقاً همان مشکل «رمز ثابت در کد عمومی» که قبلاً رفعش کرده بودیم
   دوباره برمی‌گردد.

هر دو فایل عمداً از commit های اخیر بیرون نگه داشته شده‌اند (فقط local، uncommitted).
کاربر گفته «فعلاً همینه، بعداً عوضش می‌کنم» — **یعنی این موضوع باز است**. سشن بعدی قبل
از هر commit جدید حتماً چک کند این دو فایل staged نشوند، مگر کاربر صریحاً بخواهد.
